...
class Hart85(Disaggregator):
    ...
    def compute_rmse(self, gt, pred):
        rms_error = {}
        for app_idx, app_name in enumerate(gt.columns):
            gt_app = gt.iloc[:, app_idx]
            pred_app = pred.iloc[:, app_idx]
            if pred_app.empty:
                continue

            df_app = pd.DataFrame(
                {'gt': gt_app, 'pred': pred_app}, index=gt_app.index).dropna()
            if not df_app.empty:
                app_rms_error = np.sqrt(
                    mean_squared_error(df_app['gt'], df_app['pred']))
            else:
                app_rms_error = np.NaN

            rms_error['{} (#{})'.format(app_name, app_idx)] = app_rms_error
    ...
    def best_matched_appliance(self, submeters, pred_df):
        ...
        for pred_appliance in pred_df.columns:
            ...
            for appliance in submeters_df.columns:                
                # if (not np.isnan(new_df[pred_appliance][0]) and not np.isnan(new_df[appliance][0]) and \
                #     not np.isnan(new_df[pred_appliance][1]) and not np.isnan(new_df[appliance][1]) and \
                #     not np.isnan(new_df[pred_appliance][2]) and not np.isnan(new_df[appliance][2])):
                #     temp_value = (
                #         np.sqrt(
                #             mean_squared_error(
